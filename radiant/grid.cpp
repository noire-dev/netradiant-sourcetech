...
void maxGridCoordPowerImport( int value ){
	...
	g_maxGridCoord = pow( 2.0, std::clamp( g_maxGridCoordPower, 0, 7 ) + 12 );
...
void maxGridCoordPowerExport( const IntImportCallback& importer ){
	importer( std::clamp( g_maxGridCoordPower, 0, 7 ) );
}
...
void Grid_constructPreferences( PreferencesPage& page ){
	...
	{
		const char* coords[] = { "4096", "8192", "16384", "32768", "65536", "131072", "262144", "524288" };
